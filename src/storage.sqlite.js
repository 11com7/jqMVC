...
var SqliteStorageAdapter = (function($) {
    ...
    SqliteStorageAdapter.prototype =
        {
            ...
            _savePrepareSql: function(tableName, columns, isNew) {
                ...
                // UPDATE statement
                    ...
                    columns.forEach(function(col, t) {
                        tuple.push(col + "=" + placeholder[t]);
                    });
            ...
            _prepareDbQuery: function(obj) {
                if (!$.DbQuery) {
                    throw new Error("$.DbQuery is missing! Please load class.db_query.js.");
                }
    ...
    function _getWriteColumns(columnsOrTableName)
    {
        ...
        return columns.filter(function(el) {
            return (_protectedColumns.indexOf(el) === -1);
        });
    }
